...
using RinsTrap.UI.Elements.Editor;
...
namespace RinsTrap.UI.ViewModels.Settings
{
    public class AppearanceViewModel : NotifyPropertyChangedViewModel
    {
        ...
        public Visibility ChooseSkyboxVisibility => String.IsNullOrEmpty(SkyboxTask.NewState) ? Visibility.Visible : Visibility.Collapsed;

        public Visibility RemoveSkyboxVisibility => String.IsNullOrEmpty(SkyboxTask.NewState) ? Visibility.Collapsed : Visibility.Visible;
        ...
        private void ChooseSkybox()
        {
            ...
            var missing = SkyboxTask.GetMissingFaces(dialog.SelectedPath);

            if (missing.Any())
            {
                Frontend.ShowMessageBox(Strings.Skybox_Errors_MissingFaces.Replace("{faces}", String.Join(", ", missing)), MessageBoxImage.Error);
                return;
            }
            ...
            set
            {
                if (String.IsNullOrEmpty(value))
                {
                    if (App.Settings.Prop.BootstrapperIcon == BootstrapperIcon.IconCustom)
                        App.Settings.Prop.BootstrapperIcon = BootstrapperIcon.IconRinsTrap;
                }
                else
                {
