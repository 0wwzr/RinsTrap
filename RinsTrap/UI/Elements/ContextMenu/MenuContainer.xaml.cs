...
namespace RinsTrap.UI.Elements.ContextMenu
{
    /// <summary>
    ...
    public partial class MenuContainer
    {
        ...
        {
            const string LOG_IDENT = "MenuContainer::TakeScreenshotMenuItem_Click";

            try
            {
                if (_watcher.ProcessId == 0)
                    throw new Exception("Roblox process not found");

                using var process = Process.GetProcessById(_watcher.ProcessId);

                if (process.HasExited)
                    throw new Exception("Roblox process has exited");
                ...
                if (width <= 0 || height <= 0)
                    throw new Exception("Window is minimized or has invalid bounds");

                Directory.CreateDirectory(Paths.Screenshots);

                string fileName = $"Roblox-{DateTime.Now:yyyyMMdd-HHmmss}.png";
                string filePath = Path.Combine(Paths.Screenshots, fileName);
                ...
                App.Logger.WriteLine(LOG_IDENT, $"Saved screenshot to '{filePath}'");

                Frontend.ShowBalloonTip(
                    Strings.Notifications_Screenshot_Title,
                    String.Format(Strings.Notifications_Screenshot_Text, Paths.Screenshots),
            ...
            catch (Exception ex)
            {
                ...
                Frontend.ShowMessageBox(string.Format(Strings.ContextMenu_ScreenshotFailed, ex.Message), MessageBoxImage.Error);
            }
        }
