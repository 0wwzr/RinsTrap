...
using RinsTrap.Models.RobloxApi;
using DiscordRPC;
...
namespace RinsTrap.Integrations
{
    public class DiscordRichPresence : IDisposable
    {
        ...
        private void ProcessSetRichPresence(Message message, bool implicitUpdate)
        {
            ...
            Models.RinsTrapRPC.RichPresence? presenceData;
            ...
            try
            {
                presenceData = message.Data.Deserialize<Models.RinsTrapRPC.RichPresence>();
            }
            catch (Exception)
            {
        ...
        public async Task<bool> SetCurrentGame()
        {
            ...
            // Discord caps the details line at 128 characters, so trim the game name
            // instead of the branding to make sure it always stays visible
            string branding = $" with {App.ProjectName}";
            int maxNameLength = 128 - branding.Length;

            if (universeName.Length > maxNameLength)
                universeName = universeName[..(maxNameLength - 3)].TrimEnd() + "...";

            _currentPresence = new DiscordRPC.RichPresence
            {
                Details = $"{universeName}{branding}",
                State = status,
                Timestamps = App.Settings.Prop.ShowPlaytimeOnRichPresence ? new Timestamps { Start = timeStarted.ToUniversalTime() } : null,
                Buttons = GetButtons(),
                Assets = new Assets
                {
                    LargeImageKey = icon,
            ...
            // this is used for configuration from RinsTrapRPC
        ...
        public Button[] GetButtons()
        {
            ...
            if (App.Settings.Prop.ShowGamePageButton)
            {
                buttons.Add(new Button
                {
                    Label = "See game page",
